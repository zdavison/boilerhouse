...
export interface SnapshotMetricsDeps {
	...
	/** Returns per-tenant snapshot disk usage. */
	getDiskPerTenant: () => Array<{ tenant: string; workload: string; bytes: number }>;
...
export function instrumentSnapshots(meter: Meter, deps: SnapshotMetricsDeps): SnapshotMetrics {
	...
	const diskPerTenant = meter.createObservableGauge("boilerhouse.snapshot.disk.per_tenant", {
		description: "Snapshot disk usage per tenant",
		unit: "By",
	});

	diskPerTenant.addCallback((result) => {
		for (const { tenant, workload, bytes } of deps.getDiskPerTenant()) {
	...
	return { createDuration, creates, goldenQueueDepth, diskTotal, diskAvgPerTenant, diskPerTenant, snapshotCount };
}
