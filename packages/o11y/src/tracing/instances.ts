...
interface TraceableInstanceManager {
	...
	restoreFromSnapshot(ref: unknown, tenantId: string): Promise<unknown>;
}
...
export function wrapInstanceManager<T extends TraceableInstanceManager>(
	...
	return new Proxy(manager, {
		get(target, prop, receiver) {
			...
			if (prop === "restoreFromSnapshot") {
				return async (ref: unknown, tenantId: string) => {
					return tracer.startActiveSpan("instance.restore", async (span) => {
						const snapshotRef = ref as { id?: string; type?: string };
						if (snapshotRef.id) span.setAttribute("snapshot.id", snapshotRef.id);
						if (snapshotRef.type) span.setAttribute("snapshot.type", snapshotRef.type);
						span.setAttribute("tenant.id", tenantId);
						try {
							const result = await target.restoreFromSnapshot(ref, tenantId);
							const handle = result as { instanceId?: string };
							if (handle.instanceId) span.setAttribute("instance.id", handle.instanceId);
							return result;
						} catch (err) {
