...
beforeEach(() => {
	...
	instanceManager = new InstanceManager(runtime, db, createTestAudit(db, nodeId), nodeId);
	poolManager = new PoolManager(instanceManager, runtime, db, { healthChecker: alwaysHealthy });
});
...
describe("PoolManager", () => {
	describe("prime()", () => {
		...
		test("destroys instance on health check failure (status=destroyed, not in pool)", async () => {
			...
			const failingPm = new PoolManager(instanceManager, runtime, db, { healthChecker: failingHealthChecker });

			await expect(failingPm.prime(workloadId2)).rejects.toThrow("Health check failed");

			// Instance should be in "destroyed" state and no longer in warming/ready pool
			const remaining = db.select().from(instances).where(eq(instances.workloadId, workloadId2)).all();
			expect(remaining.every((r) => r.status === "destroyed")).toBe(true);
			expect(remaining.every((r) => r.poolStatus !== "warming" && r.poolStatus !== "ready")).toBe(true);
		});
