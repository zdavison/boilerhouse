...
for (const rt of availableRuntimes()) {
	...
	describe(`[${rt.name}] concurrent tenants`, () => {
		...
		test("3 tenants claim and release in parallel", async () => {
			...
			// Step 3: Verify at least 3 active instances (pool may pre-warm extras)
			...
			// Step 6: All tenant instances released (pool instances may remain)
			...
			const tenantInstances = activeInstances2.filter(
				(i: { poolStatus: string | null }) => i.poolStatus === null || i.poolStatus === "acquired",
			);
			expect(tenantInstances.length).toBe(0);
