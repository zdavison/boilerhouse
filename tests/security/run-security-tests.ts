...
import { generateTriggerId } from "../../packages/core/src/index";
import { triggers } from "../../packages/db/src/index";
import type { DrizzleDb } from "../../packages/db/src/index";
import { startE2EServer } from "../e2e/e2e-helpers";
import {
	Dispatcher,
	SessionManager,
	createWebhookRoutes,
...
async function startSecurityServer() {
	...
	const server = await startE2EServer("fake", {
		onDbReady: (db) => seedTriggers(db),
	});

	// Start a standalone trigger gateway that proxies claims to the API via HTTP
	const triggerGateway = startTriggerGateway(server.baseUrl, server.db);
