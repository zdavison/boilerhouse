...
export function tenantRoutes(deps: RouteDeps) {
	const { db, tenantManager, log } = deps;
	...
	return new Elysia({ name: "tenants" })
		.post("/tenants/:id/claim", async ({ params, body, set }) => {
			...
			log?.info(
				{ tenantId, instanceId: result.instanceId, workloadId: workloadRow.workloadId, source: result.source, latencyMs: Math.round(result.latencyMs) },
				"Tenant claimed",
			);
		...
		.post("/tenants/:id/release", async ({ params, body, set }) => {
			...
			log?.info({ tenantId, workloadId: workloadRow.workloadId }, "Tenant released");
		...
		.get("/tenants/:id", ({ params, set }) => {
			...
			return tenantRows.map((tenantRow) => {
				...
				return {
					...
					dataOverlayRef: tenantRow.dataOverlayRef ?? null,
